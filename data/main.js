        ...
        function initRover3D() {
            ...
            // --- Modelo del Rover (Diseño Premium) ---
            const roverGroup = new THREE.Group();

            // === CHASIS PRINCIPAL (forma de cuña) ===
            const chassisShape = new THREE.Shape();
            chassisShape.moveTo(-1.0, -1.5);
            chassisShape.lineTo(1.0, -1.5);
            chassisShape.lineTo(1.1, 1.2);
            chassisShape.lineTo(0.8, 1.6);
            chassisShape.lineTo(-0.8, 1.6);
            chassisShape.lineTo(-1.1, 1.2);
            chassisShape.lineTo(-1.0, -1.5);
            const chassisExtrudeSettings = { depth: 0.5, bevelEnabled: true, bevelThickness: 0.05, bevelSize: 0.05, bevelSegments: 3 };
            ...
            // === CUBIERTA SUPERIOR (techo aerodinámico) ===
            const roofGeo = new THREE.BoxGeometry(1.8, 0.15, 2.6);
            ...
            const solarGeo = new THREE.BoxGeometry(1.5, 0.04, 1.8);
            ...
            solar.position.y = 1.25;
            solar.position.z = -0.2;
            roverGroup.add(solar);

            // Líneas del panel solar (celdas)
            const cellMat = new THREE.MeshStandardMaterial({
                color: 0x283593, metalness: 0.5, roughness: 0.3,
                emissive: 0x1565c0, emissiveIntensity: 0.1
            });
            for (let i = -0.6; i <= 0.6; i += 0.3) {
                const cellLine = new THREE.Mesh(
                    new THREE.BoxGeometry(0.02, 0.05, 1.7),
                    cellMat
                );
                cellLine.position.set(i, 1.27, -0.2);
                roverGroup.add(cellLine);
            }

            // === CABINA DE SENSORES (cúpula translúcida) ===
            const domeGeo = new THREE.SphereGeometry(0.4, 16, 12, 0, Math.PI * 2, 0, Math.PI / 2);
            const domeMat = new THREE.MeshStandardMaterial({
                color: 0x38bdf8, metalness: 0.1, roughness: 0.0,
                transparent: true, opacity: 0.35,
                emissive: 0x38bdf8, emissiveIntensity: 0.2
            });
            const dome = new THREE.Mesh(domeGeo, domeMat);
            dome.position.set(0, 1.22, 0.6);
            roverGroup.add(dome);

            // Sensor dentro de la cúpula (cámara)
            const camGeo = new THREE.BoxGeometry(0.15, 0.15, 0.2);
            const camMat = new THREE.MeshStandardMaterial({
                color: 0x0f172a, metalness: 0.9, roughness: 0.1
            });
            const cam = new THREE.Mesh(camGeo, camMat);
            cam.position.set(0, 1.32, 0.6);
            roverGroup.add(cam);

            // Lente de la cámara
            const lensGeo = new THREE.CylinderGeometry(0.05, 0.06, 0.08, 12);
            ...
            lens.position.set(0, 1.32, 0.72);
            roverGroup.add(lens);

            // === BRAZOS DE SUSPENSIÓN + RUEDAS ===
            const wheelGeo = new THREE.CylinderGeometry(0.38, 0.38, 0.25, 20);
            const tireMat = new THREE.MeshStandardMaterial({
                color: 0x1a1a2e, metalness: 0.3, roughness: 0.9
            });
            const rimGeo = new THREE.CylinderGeometry(0.22, 0.22, 0.26, 12);
            const rimMat = new THREE.MeshStandardMaterial({
                color: 0xb8860b, metalness: 0.9, roughness: 0.1
            });
            const hubGeo = new THREE.CylinderGeometry(0.08, 0.08, 0.28, 8);
            const hubMat = new THREE.MeshStandardMaterial({
                color: 0xc0c0c0, metalness: 0.95, roughness: 0.05
            });
            const armGeo = new THREE.BoxGeometry(0.12, 0.08, 0.7);
            const armMat = new THREE.MeshStandardMaterial({
                color: 0x64748b, metalness: 0.7, roughness: 0.3
            });
            ...
            wheelPositions.forEach(pos => {
                ...
                arm.position.set(pos.x * 0.55, 0.5, pos.z);
                roverGroup.add(arm);
            });

            // === FAROS FRONTALES (2 LEDs) ===
            ...
            [-0.55, 0.55].forEach(x => {
                ...
                hl.position.set(x, 0.75, 1.62);
                roverGroup.add(hl);
            });

            // === LUCES TRASERAS (2 LEDs rojos) ===
            ...
            [-0.55, 0.55].forEach(x => {
                ...
                tl.position.set(x, 0.75, -1.52);
                roverGroup.add(tl);
            });

            // === PARACHOQUES FRONTAL ===
            const bumperGeo = new THREE.BoxGeometry(1.8, 0.12, 0.08);
            const bumperMat = new THREE.MeshStandardMaterial({
                color: 0x94a3b8, metalness: 0.8, roughness: 0.2
            });
            const bumper = new THREE.Mesh(bumperGeo, bumperMat);
            bumper.position.set(0, 0.5, 1.65);
            roverGroup.add(bumper);

            // === ANTENA DE COMUNICACIONES ===
            const antennaGeo = new THREE.CylinderGeometry(0.02, 0.025, 1.5, 8);
            const antennaMat = new THREE.MeshStandardMaterial({
                color: 0xcbd5e1, metalness: 0.8, roughness: 0.2
            });
            const antenna = new THREE.Mesh(antennaGeo, antennaMat);
            antenna.position.set(0.6, 2.0, -0.9);
            roverGroup.add(antenna);

            // Bola emisora (punta de antena)
            const antBallGeo = new THREE.SphereGeometry(0.07, 12, 12);
            const antBallMat = new THREE.MeshStandardMaterial({
                color: 0xef4444, emissive: 0xef4444, emissiveIntensity: 0.8
            });
            const antBall = new THREE.Mesh(antBallGeo, antBallMat);
            antBall.position.set(0.6, 2.8, -0.9);
            roverGroup.add(antBall);

            // === INDICADOR DE DIRECCIÓN (flecha superior) ===
            ...
            arrow.position.set(0, 1.35, 1.45);
            roverGroup.add(arrow);

            // === DETALLES LATERALES (líneas de acento) ===
            const accentMat = new THREE.MeshStandardMaterial({
                color: 0x38bdf8, emissive: 0x38bdf8, emissiveIntensity: 0.4
            });
            [-1.05, 1.05].forEach(x => {
                const stripe = new THREE.Mesh(
                    new THREE.BoxGeometry(0.03, 0.06, 2.0),
                    accentMat
                );
                stripe.position.set(x, 0.85, 0);
                roverGroup.add(stripe);
            });

            // Luz de estado (punto de estado en la cubierta)
            const ledMat = new THREE.MeshStandardMaterial({
                color: 0x38bdf8, emissive: 0x38bdf8, emissiveIntensity: 1.0,
                transparent: true, opacity: 0.9
            });
